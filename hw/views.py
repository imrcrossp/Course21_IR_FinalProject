					...
					for l in range(len(tokens_tag)):
						...
						for m in target_col:
							if (m == temp[l] and not find_s):
								...
								replaced_s = k.replace(" " + temp[l].split('%')[0], " <span style=\"background-color:yellow\">" + temp[l].split('%')[0] + "</span>")

								# Replace the title keyword
								insensitive = re.compile(re.escape(temp[l].split('%')[0]+" "), re.IGNORECASE)
								replaced_s = insensitive.sub("<span style=\"background-color:yellow\">" + temp[l].split('%')[0].capitalize() + "</span> ", replaced_s)
